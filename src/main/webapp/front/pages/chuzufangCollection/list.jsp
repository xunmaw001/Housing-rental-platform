            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">出租房收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="chuzufangName" id="chuzufangName" placeholder="出租房名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                             
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">出租房类型</label>
                              <div class="layui-input-inline">
                                  <select name="chuzufangTypes" id="chuzufangTypes" lay-filter="chuzufangTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in chuzufangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <div class="row">
                            <div class="col-md-3" v-for="(item,index) in dataList" >
                                <div class="galry-grid grid-top-rgt">
                                    <i @click="deleteChuzufangCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <a href="javascript:return false;"  @click="jumpCheck('../chuzufang/detail.jsp?id='+item.chuzufangId , item.chuzufangDelete == null?'':item.chuzufangDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img class="img-responsive zoom-img" :src="item.chuzufangPhoto?item.chuzufangPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                        <div class="w3agile-text">
                                            <h5>
                                                <span>{{item.chuzufangName}}</span>
                                                <br/>
                                                <span v-if="item.chuzufangNewMoney">
                                                    {{item.chuzufangNewMoney}} RMB
                                                </span>
                                                <span v-else>
                                                    {{item.chuzufangValue}}
                                                </span>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            chuzufangCollectionTypesList : [],
        // 级联表字典表数据
            chuzufangTypesList : [],
            shangxiaTypesList : [],
            chuzufangCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteChuzufangCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('chuzufangCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function chuzufangCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chuzufang_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chuzufangCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function chuzufangTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chuzufang_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chuzufangTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#chuzufangName').val() != null && jquery('#chuzufangName').val() != "" && jquery('#chuzufangName').val() != "null" ) {
                param['chuzufangName'] = jquery('#chuzufangName').val();
            }
                                                             
            var chuzufangTypes = $("#chuzufangTypes").find("option:selected").val();
            if (chuzufangTypes != null && chuzufangTypes != "" && chuzufangTypes != "null") {
                            param['chuzufangTypes'] = chuzufangTypes;
                        }
                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['chuzufangCollectionTypes'] = 1;

            // 获取列表数据
            http.request('chuzufangCollection/page', 'get', param, function(res) {
