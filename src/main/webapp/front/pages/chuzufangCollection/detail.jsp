...
<body>
        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.chuzufangCollectionName}}</cite></a>
                </span>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.chuzufangCollectionPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.chuzufangCollectionTypes">类型：
                                {{detail.chuzufangCollectionValue}}
                            </li>
                        </ul>
        ...
        <div class="col-lg-4" v-if="chuzufangCollectionRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in chuzufangCollectionRecommendList" v-bind:key="index" @click="jump('../chuzufangCollection/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.chuzufangCollectionPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.chuzufangCollectionName}}</a>
                                <h6 v-if="item.chuzufangCollectionNewMoney" class="post-title"><a style="color: red">{{item.chuzufangCollectionNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            //系统推荐
            chuzufangCollectionRecommendList: [],
            dataList: [],
            // 当前详情页表
            detailTable: 'chuzufangCollection',
        },
        //  清除定时器
        destroyed: function () {
            // 不知道具体作用
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.chuzufangCollectionName;
           // 轮播图片
            vue.swiperList = vue.detail.chuzufangCollectionPhoto ? vue.detail.chuzufangCollectionPhoto.split(",") : [];
            var swiperItemHtml = '';
        ...
        http.request(`chuzufangCollection/list`, 'get', {
            page: 1,
            limit: 5,
            chuzufangCollectionTypes: vue.detail.chuzufangCollectionTypes,




        }, function (res) {
            vue.chuzufangCollectionRecommendList = res.data.list;
        });
