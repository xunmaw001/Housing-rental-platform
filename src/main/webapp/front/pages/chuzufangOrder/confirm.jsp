		...
		<script>
			var vue = new Vue({
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.chuzufangNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						let data={
                            chuzufangs:localStorage.getItem('chuzufangs'),
                            yonghuId: localStorage.getItem('userid'),
						}
                        // 获取商品详情信息
                        layui.http.request(`chuzufangOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../chuzufangOrder/list.jsp';
							}
                       		 window.location.href='../chuzufangOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取出租房购买的清单列表
				var chuzufangs = localStorage.getItem('chuzufangs');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(chuzufangs);
