            ...
            <div class="ng-inmod-ini">
                ...
                <div class="ng-incontlst ng-fault ng-container ng-ai-drive1_map" aos="fade-up">
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in chuzufangList" class="col-md-3">
                                <div class="gallery-demo" @click="jumpCheck('../chuzufang/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.chuzufangPhoto?item.chuzufangPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.chuzufangName}}
                                        <span v-if="item.chuzufangNewMoney"> - ￥{{item.chuzufangNewMoney}}</span>
                                        <span v-else> - {{item.chuzufangValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery'], function () {
        ...
        http.request(`chuzufang/list`, 'get', {
            ...
            vue.chuzufangList = res.data.list;
        });
    });
</script>
